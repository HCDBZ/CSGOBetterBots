...
public Plugin myinfo = {
    ...
    author = "Tasty cup",
    description = "简单的暂停系统，每队4次暂停机会，支持原生函数调用",
...
public void Event_RoundStart(Event event, const char[] name, bool dontBroadcast) {
    ...
    if (iCurrentRound == 0) {
        ...
        g_iTeamPausesLeft[2], g_iTeamPausesLeft[3];
    }
}
...
public Action Command_Pause(int client, int args) {
    ...
    if (g_bGamePaused) {
        PrintToChat(client, " [\x04CSGO\x01] 游戏已经暂停中");
        return Plugin_Handled;
    }
    
    int team = GetClientTeam(client);
    if (team != 2 && team != 3) {
        PrintToChat(client, " [\x04CSGO\x01] 只有队伍成员可以暂停");
        return Plugin_Handled;
    }
    
    if (g_iTeamPausesLeft[team] <= 0) {
        PrintToChat(client, " [\x04CSGO\x01] 你的队伍暂停次数已用完");
        return Plugin_Handled;
    }
    ...
    if (args > 0) {
        ...
        if (pauseTime < 1) {
            PrintToChat(client, " [\x04CSGO\x01] 暂停时间必须大于0秒");
            return Plugin_Handled;
        }
...
void ExecutePause(int client, int team, int pauseTime) {
    ...
    if (pauseTime == DEFAULT_PAUSE_TIME) {
        PrintToChatAll(" [\x04CSGO\x01] %s 暂停了比赛，%s队暂停剩余次数\x04%d\x01次", 
            clientName, teamName, g_iTeamPausesLeft[team]);
    } else {
        PrintToChatAll(" [\x04CSGO\x01] %s 暂停了比赛 (\x04%d\x01秒)，%s队暂停剩余次数\x04%d\x01次", 
            clientName, pauseTime, teamName, g_iTeamPausesLeft[team]);
    }
